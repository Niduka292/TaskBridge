  ...
  return (
    <div className="flex flex-col items-center justify-center py-16 gap-4">
      ...
      <Link href={tab === 'poster' ? '/tasks/new' : '/tasks'}>
        <Button
          size="sm"
          variant={tab === 'poster' ? 'default' : 'outline'}
        >
          {tab === 'poster'
            ? 'Post a task'
            : 'Browse tasks'}
...
export default function DashboardPage() {
  ...
  const [tab, setTab] =
    useState<'poster' | 'freelancer'>('poster')
  ...
  useEffect(() => {
    async function loadDashboard() {
      ...
      try {
        ...
        setError(
          'Could not load your dashboard tasks.'
        )
      } finally {
        setLoading(false)
      }
  ...
  // ── Separate poster/freelancer tasks ───────

  const postedTasks = allTasks.filter(
    task => task.posterId === userId
  )

  const freelancerJobs = allTasks.filter(
    task => task.assignedTo === userId
  )

  // Tasks displayed according to selected tab
  const visibleTasks =
    tab === 'poster'
      ? postedTasks
      : freelancerJobs

  // ── Counts for selected tab ────────────────

  const counts = {
    open: visibleTasks.filter(
      task => task.status === 'OPEN'
    ).length,

    active: visibleTasks.filter(
      task =>
      ...
      <div className="max-w-4xl mx-auto px-4 py-12 space-y-6">
        ...
        <div>
          ...
          <p className="text-zinc-500 text-sm mt-1">
            Manage your posted tasks and freelance work.
          </p>
        </div>
