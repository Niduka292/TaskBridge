...
import com.taskbridge.taskservice.event.BidReceivedEvent;
...
public class BidService {
    ...
    public BidResponse submitBid(UUID taskId, BidRequest request, UUID bidderId, String bidderName, String bidderAvatar) {
        ...
        BidReceivedEvent event = new BidReceivedEvent(
                task.getId(),
                task.getTitle(),
                task.getPosterId(),
                saved.getId(),
        ...
        // Publish only after the bid row (and bidCount bump) actually commit —
        // same reasoning as BID_ACCEPTED below.
        TransactionSynchronizationManager.registerSynchronization(
                new TransactionSynchronization() {
                    @Override
                    public void afterCommit() {
                        eventPublisher.publish("BID_RECEIVED", event);
                    }
                }
        );
    ...
    public BidResponse acceptBid(UUID bidId, UUID callerId) {
        ...
        // Capture who else was still pending BEFORE the bulk update wipes their
        // status — this is the only chance to know who to notify as "rejected".
        List<UUID> rejectedBidderIds = bidRepository.findByTaskId(task.getId()).stream()
                .filter(b -> !b.getId().equals(bid.getId()) && b.getStatus() == BidStatus.PENDING)
                .map(Bid::getBidderId)
                .toList();
